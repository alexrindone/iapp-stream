...
export default class Navbar extends React.Component {
    ...
    getLiveStreams() {
        axios.get('http://35.175.134.119:' + config.rtmp_server.http.port + '/api/streams')
            .then(res => {
    ...
    render() {
        let streams;
        if (this.state.live_streams.length > 0) {
            streams = this.state.live_streams.map((stream, index) => {
                return (
            ...
            streams = <div><p>No streams are currently available.</p></div>;
        }
